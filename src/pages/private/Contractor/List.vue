<template>
  <div>
    <AddView :button-text="'Добавить контрагента'">
      <template v-slot:dialogLabel>Добавление контрагента</template>
...
function handleRowClick(e, row) {
  route.push(`/contractor/detail/${row.id}`);
}
function handleCreate() {
  ...
  store.dispatch("contractors/create", combinedObject).then(
    () => {
      $q.notify({
        message: "Успешно создан контрагент",
        color: "positive",
        position: "top",
      });
...
onMounted(() => {
  store.dispatch("contractors/getAll");
});
const rows = computed(() => store.state.contractors.list);
