<template>
    <div>
        <AddView :button-text="'Добавить инвентаризацию'">
            <template v-slot:dialogLabel>Добавление инвентаризации</template>
...
function handleRowClick(e, row) {
    route.push(`/inventory/detail/${row.id}`);
}
function handleCreate() {
    ...
    store.dispatch('inventory/create', combinedObject).then(
        () => {
            $q.notify({
...
onMounted(() => {
    store.dispatch('inventory/getAll');
});
const rows = computed(() => store.state.inventory.list);
